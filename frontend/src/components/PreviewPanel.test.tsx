...
describe("PreviewPanel", () => {
	...
	it("shows error message when failed", () => {
		...
		expect(
			screen.getByText(/build failed: Dockerfile not found/),
		).toBeInTheDocument();
	});
	...
	it("calls create mutation with worktree name", async () => {
		...
		await user.type(
			screen.getByLabelText(/worktree name/i),
			"my-worktree",
		);
	...
	it("clears input after successful creation", async () => {
		...
		expect(
			(screen.getByLabelText(/worktree name/i) as HTMLInputElement).value,
		).toBe("");
	});
