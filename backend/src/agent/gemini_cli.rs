...
use crate::agent::executor::{
    run_subprocess, validate_config, AgentConfig, AgentExecutor, AgentHandle, AgentOutputEvent,
};
...
impl AgentExecutor for GeminiCliExecutor {
    async fn start(&self, config: AgentConfig) -> AppResult<AgentHandle> {
        ...
        run_subprocess(cmd, &config.prompt, "gemini", parse_gemini_stream_line).await
    }
}
